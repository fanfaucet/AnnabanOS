...
- `annaban_maritime/core/eta.py` estimates ETA windows from vessel speed, destination coordinates, weather, congestion, current, and fuel context.
- `annaban_maritime/core/routing.py` scores and selects routes using configurable weights for distance, weather, congestion, ecological risk, and priority signals.
- `annaban_maritime/core/routing.py` also includes deterministic placeholder route generation for direct, weather-avoidance, and eco-avoidance candidates.
...
- `annaban_maritime/api/main.py` exposes Pydantic-validated FastAPI endpoints for ETA, route checking, route generation, and best-route selection.
